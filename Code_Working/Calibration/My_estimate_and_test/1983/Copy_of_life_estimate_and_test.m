...
% stack all data moments: CAREFUL below to stack the model moments the SAME way
  sel_sampleM       = [ 1.13; 2.63; 3.93; 0.606; 0.511; 0.503];
...
composed_26_55 = NaN*zeros(99,tot_cases);
composed_26_35 = NaN*zeros(99,tot_cases);
composed_36_45 = NaN*zeros(99,tot_cases);
composed_46_55 = NaN*zeros(99,tot_cases);
...
for iprepcase = 1:tot_cases;
    for iperc = 1:99;
            composed_26_55(iperc,iprepcase) = sorted_cs_x_prime(round(max(size(sorted_cs_x_prime))*iperc/100),iprepcase);
            composed_26_35(iperc,iprepcase) = sorted_cs_x_prime(round(max(size(sorted_cs_x_prime))*iperc/100),iprepcase);
            composed_36_45(iperc,iprepcase) = sorted_cs_x_prime(round(max(size(sorted_cs_x_prime))*iperc/100),iprepcase);
            composed_46_55(iperc,iprepcase) = sorted_cs_x_prime(round(max(size(sorted_cs_x_prime))*iperc/100),iprepcase);
    end;
end; % of for writing percentiles from all cases

% Mean up to 90th percentile 
mean_x_26_35_90th = mean(composed_26_35(1:90,:));
mean_x_36_45_90th = mean(composed_36_45(1:90,:));
mean_x_46_55_90th = mean(composed_46_55(1:90,:));

% Gini up to 90th percentile 
for igini = 1: tot_cases
    gini_x_26_35_90th(igini) = my_gini(composed_26_35(1:90,igini));
    gini_x_36_45_90th(igini) = my_gini(composed_36_45(1:90,igini));
    gini_x_46_55_90th(igini) = my_gini(composed_46_55(1:90,igini));
end

        
% stack all model moments: CAREFUL to stack the model moments the
% SAME way as the data moments above
 sel_simM       = [ mean_x_26_35_90th; mean_x_36_45_90th; mean_x_46_55_90th; gini_x_26_35_90th; gini_x_36_45_90th; gini_x_46_55_90th];
