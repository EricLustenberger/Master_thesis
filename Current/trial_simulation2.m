...
%% import parameters from 2004 calibration
...
x_t     = NaN*zeros(pop_size,size(Y_ms_j,2)-1);
c_t     = NaN*zeros(pop_size,size(Y_ms_j,2)-1);
...
%             if t==1;
%                 x_t(:,t)         = (1+r)*a_initial + (1-delta_)*d_initial;
...
%             end; % on initial period
            ...
            for imarkstate = 1 : size(P_,2);
                if any(s_i_t(:,t) == imarkstate);
                       ...
                       a_t(s_i_t(:,t) == imarkstate,t+1)     = interp2(MeshX,MeshD,policies(t).a_prime(:,:,imarkstate),x_t(s_i_t(:,t) == imarkstate,t),d_t(s_i_t(:,t) == imarkstate,t));
                       d_t(s_i_t(:,t) == imarkstate,t+1)     = interp2(MeshX,MeshD,policies(t).d_prime(:,:,imarkstate),x_t(s_i_t(:,t) == imarkstate,t),d_t(s_i_t(:,t) == imarkstate,t));
