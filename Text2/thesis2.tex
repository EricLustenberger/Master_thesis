...
\multicolumn{5}{l}{%
  \begin{minipage}{9.5cm}%
    \small This table shows the gini coefficients and means for the distribution of different age groups up to and with the 90th percentile of the net-worth distribution for the baseline simulation and the simulation with an LTV of 0. The unit of the means is the average labor-earnings equivalent. 
  \end{minipage}%
}\\
\end{tabular}
\end{table}

These observations are in line with the discussion above. A tighter borrowing constraint affects the poor, who are most apparent in the youngest age group and thus the younger consumers are more exposed to the LTV. Figure \ref{downpayment_vs_baseline} illustrates this well. It shows the distributions for the two different levels of the LTV. The red line represents the results obtained from the baseline calibration and the dotted blue line the steady state distribution for the prime age sample when durables do not have collateral value. 
