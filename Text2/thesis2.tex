...
A big majority of wealth is owned by very few. Moreover, there is a substantial widening of wealth disparities since the 1970s in the U.S. \citep{kuhn2017income}. Causes and implications of these facts are rigorously debated in both academics and politics. In order to promote a more vigorous discussion of the wealth inequality and its evolution over time, it is important to understand which models can be applied to study the issue at hand and to what extend they may explain certain facts observed in the data.\\
...
For a population uniform on the values $y_{i}, i=1$ to $n$, indexed in non-decreasing order $(y_{i} \leq y_{i+1})$ the gini can be expressed as follows: 
...
where $k$ is defined such that $\sum_{j=1}^k y_{j}$.
